...
export default async function AdminDashboard() {
  const [rawCCStudents, coachStudentData] = await Promise.all([
  ...
  const totalBatches = new Set(coachStudentData.map(r => r.batch_name)).size
  ...
  const stats = [
    ...
    { label: 'Active Batches', value: totalBatches,       icon: '📚', color: 'text-green-600', href: '/admin/users' },
    { label: 'Avg Win Rate',   value: (() => {
      const valid = ccStudents.filter(s => s.matches_played > 0)
      if (!valid.length) return '—'
      const avg = valid.reduce((sum, s) => sum + Number(s.win_percentage), 0) / valid.length
      return `${avg.toFixed(0)}%`
    })(),                                                   icon: '🏆', color: 'text-amber-500', href: '/admin/analytics' },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: 'All Students', icon: '👥', href: '/admin/analytics' },
            { label: 'All Coaches',  icon: '🏅', href: '/coach' },
            { label: 'Batches',      icon: '📚', href: '/admin/users' },
            { label: 'Analytics',   icon: '📊', href: '/admin/analytics' },
          ].map(a => (
            <Link key={a.label} href={a.href}
              className="flex flex-col items-center gap-2 p-4 bg-gray-50 rounded-xl hover:bg-primary/5 border border-gray-100 hover:border-primary/20 transition text-center">
