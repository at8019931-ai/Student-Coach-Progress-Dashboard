...
const coachNav: NavItem[] = [
  ...
  { label: 'Classes',     href: '/coach/classes',      icon: '📅' },
]
...
export default function Sidebar({ role, userName, avatarUrl }: SidebarProps) {
  ...
  return (
    <aside className="hidden md:flex flex-col w-60 shrink-0 bg-white border-r border-gray-100 min-h-screen">
      ...
      <div className="flex items-center gap-3 px-6 py-5 border-b border-gray-100">
        <div className="w-9 h-9 rounded-xl bg-primary flex items-center justify-center text-white text-xl shadow-sm">
          ♟
        </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        {navItems.map(item => {
          const active = item.href === '/student' || item.href === '/coach' || item.href === '/admin'
            ? pathname === item.href
